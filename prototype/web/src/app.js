...
const Game = (() => {
  ...
  let capMesh, capShadow, batter, pitcher;
  ...
  let capState = "idle";
  let cap = { z: 0, t: 0, freq: 1, ampX: 0, ampY: 0, phase: 0, vy: 0, vz: 0, vx: 0 };
  let swung = false;
  ...
  function init() {
    ...
    // Vitilla (tapa = cilindro plano)
    capMesh = new THREE.Mesh(
      new THREE.CylinderGeometry(0.4, 0.4, 0.14, 22),
  ...
  function nextPitch() {
    ...
    cap.freq = 2.2 + Math.random() * 2.2;
    cap.ampX = (powerUpActive ? 0.7 : 1.3) + Math.random() * 0.6;   // baile horizontal
    cap.ampY = 0.4 + Math.random() * 0.5;                            // flotación vertical
  ...
  function launchHit(txt, color, addRuns, power) {
    ...
    cap.vy = power * 0.6;
    cap.vz = -power;          // sale hacia el outfield
  ...
  function gilla(txt) {
    ...
    capMesh.visible = false; capShadow.visible = false;
    setMessage(txt, "#e23b3b");
    updateHUD();
    if (gillas >= 3) setTimeout(gameOver, 900); else setTimeout(nextPitch, 1000);
  }
  function endBall(wasHit, delay) {
    if (!wasHit) { capState = "done"; capMesh.visible = false; capShadow.visible = false; }
    setTimeout(() => { if (capState !== "incoming") nextPitch(); }, delay);
  }

  function gameOver() {
    capState = "done"; capMesh.visible = false; capShadow.visible = false;
    btnSwing.disabled = false;
  ...
  function loop() {
    ...
    if (capState === "incoming") {
      cap.z += speed * dt;
      const approach = (cap.z - CAP_START_Z) / (HIT_Z - CAP_START_Z); // 0..1
      const x = Math.sin(time * cap.freq + cap.phase) * cap.ampX * (0.3 + approach * 0.7);
      const y = 1.5 + Math.sin(time * cap.freq * 1.3 + cap.phase) * cap.ampY;
      capMesh.position.set(x, y, cap.z);
      capMesh.rotation.y += dt * 9;
      capMesh.rotation.x += dt * 5;
      capShadow.position.set(x, 0.02, cap.z);
      const sc = 0.6 + approach * 0.7;
      capShadow.scale.setScalar(sc);
      if (cap.z >= CAP_END_Z) { if (!swung) gilla("¡GILLA! 🚫 La dejaste pasar"); }
    } else if (capState === "hit") {
      cap.vy -= 26 * dt; // gravedad
      capMesh.position.x += cap.vx * dt;
      capMesh.position.y += cap.vy * dt;
      capMesh.position.z += cap.vz * dt;
      capMesh.rotation.y += dt * 14;
      if (capMesh.position.y < 0.1) { capMesh.visible = false; capShadow.visible = false; }
      else { capShadow.position.set(capMesh.position.x, 0.02, capMesh.position.z); }
    }
